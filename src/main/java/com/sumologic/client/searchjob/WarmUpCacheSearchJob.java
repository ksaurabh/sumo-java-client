 ...
 */
public class WarmUpCacheSearchJob {
...
//        String url = "https://long-api.sumologic.net";
        ...
        long nowMinusFiveMinutes = now - 5 * 60 * 1000L;

        String searchJobId = sumoClient.createSearchJob(
                "* | count _sourceHost",  // This query will return all messages
                dateFormat.format(new Date(nowMinusFiveMinutes)),    // between this start time and
                dateFormat.format(new Date(now)),    // this end time, specified in ISO 8601 format
        ...
        try {
            ...
            if (messageLength > 0) {
                System.out.printf("Downloading %s messages with offset %s ", messageLength, messageOffset);
                ...
                System.out.println("No messages found yet");
            }
